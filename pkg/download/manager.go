...
func (m *Manager) DownloadFileWithContext(ctx context.Context, url, dest string) error {
	...
	if dirErr := m.ensureDirectory(filepath.Dir(dest)); dirErr != nil {
		return dirErr
	}
...
func (a *Archiver) openTarGzReader(archivePath string) (*tar.Reader, func(), error) {
	...
	cleanup := func() {
		if closeErr := gzr.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close gzip reader: %v\n", closeErr)
		}
		if closeErr := file.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close file: %v\n", closeErr)
		}
...
func (a *Archiver) ExtractZip(archivePath, destDir string) error {
	...
	defer func() {
		if closeErr := r.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close zip reader: %v\n", closeErr)
		}
	}()
...
func (a *Archiver) extractZipRegularFile(f *zip.File, path string) error {
	...
	defer func() {
		if closeErr := rc.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close archive file: %v\n", closeErr)
		}
	}()
	...
	defer func() {
		if closeErr := outFile.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close output file: %v\n", closeErr)
		}
	}()
...
func (m *Manager) InstallBinary(srcPath, envPath, binaryName string) error {
	...
	if err := m.ensureDirectory(binDir); err != nil {
		return err
	}
...
func (m *Manager) copyFile(src, dst string) error {
	...
	defer func() {
		if closeErr := sourceFile.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close source file: %v\n", closeErr)
		}
	}()
	...
	defer func() {
		if closeErr := destFile.Close(); closeErr != nil {
			fmt.Printf("⚠️  Warning: failed to close destination file: %v\n", closeErr)
		}
	}()
