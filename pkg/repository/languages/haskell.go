...
// CheckHealth checks if the Haskell environment is healthy
func (h *HaskellLanguage) CheckHealth(envPath, _ string) error {
	// Check if Haskell runtime is available
	if !h.IsRuntimeAvailable() {
		return fmt.Errorf("haskell runtime not available on system")
	}
...
func (h *HaskellLanguage) SetupEnvironmentWithRepo(
	...
	if err := h.CheckHealth(envPath, version); err == nil {
		return envPath, nil
	}
