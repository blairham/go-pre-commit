...
func TestCoursierLanguage_SetupEnvironmentWithRepo(t *testing.T) {
	...
	t.Run("UnsupportedVersion", func(t *testing.T) {
		coursierLang := NewCoursierLanguage()
		...
		// Coursier now normalizes any version to 'default' to match Python pre-commit behavior
		envPath, err := coursierLang.SetupEnvironmentWithRepo(tempDir, "2.13", tempDir, "dummy-url", []string{})
		if err != nil {
			t.Logf("SetupEnvironmentWithRepo() with version '2.13' returned: %v", err)
		}

		// Should succeed since all versions are normalized to 'default'
		if envPath == "" {
			t.Error(
				"SetupEnvironmentWithRepo() should return valid path even with non-default version (normalized to default)",
			)
		}

		// Environment directory should use 'default' version
		expectedEnvName := "coursierenv-default"
		if !strings.Contains(envPath, expectedEnvName) {
			t.Errorf(
				"SetupEnvironmentWithRepo() environment path should contain '%s', got: %s",
				expectedEnvName,
				envPath,
			)
		}
	...
	t.Run("SetupEnvironmentWithRepo_EdgeCases", func(t *testing.T) {
		...
		// Test with different versions - should all be normalized to 'default'
		testVersions := []string{"1.0", "2.0", "invalid", "1.3.6"}
		for _, version := range testVersions {
			envPath, err := coursier.SetupEnvironmentWithRepo(tempDir, version, tempDir, "dummy-url", []string{})
			if err != nil {
				t.Logf("SetupEnvironmentWithRepo() with version %s returned: %v", version, err)
			}
			// Should succeed since all versions are normalized to 'default'
			if envPath == "" {
				t.Errorf(
					"SetupEnvironmentWithRepo() with version %s should return valid path (normalized to default)",
					version,
				)
			}
		}

		// Test with empty version - should use default
		envPath, err := coursier.SetupEnvironmentWithRepo(tempDir, "", tempDir, "dummy-url", []string{})
		if err != nil {
			t.Logf("SetupEnvironmentWithRepo() with empty version returned: %v", err)
		}
		if envPath == "" {
			t.Error("SetupEnvironmentWithRepo() with empty version should return valid path (using default)")
		}
	})
}
