...
func NewTestGitRepo(t *testing.T) *TestGitRepo {
	...
	if err := gitCmd.Run(); err != nil {
		if removeErr := os.RemoveAll(tempDir); removeErr != nil {
			t.Logf("⚠️  Warning: failed to remove temp dir: %v", removeErr)
		}
...
func (r *TestGitRepo) Cleanup() {
	if err := os.RemoveAll(r.Path); err != nil {
		r.t.Logf("⚠️  Warning: failed to remove temp dir: %v", err)
	}
}
...
func (r *TestGitRepo) ChangeToRepo() func() {
	...
	return func() {
		if err := os.Chdir(oldDir); err != nil {
			r.t.Logf("⚠️  Warning: failed to change back to original directory: %v", err)
		}
	}
}
