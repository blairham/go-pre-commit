	...
	defer func() {
		...
		if resultsCount > 0 {
			if err := reportGenerator.SaveResults(); err != nil {
				t.Logf("Warning: Failed to save test results in deferred save: %v", err)
			} else {
				t.Logf("✅ Successfully saved %d test results", resultsCount)
			}

			if err := reportGenerator.GenerateReport(); err != nil {
				t.Logf("Warning: Failed to generate test report in deferred save: %v", err)
			}
		}
	}()
...
func TestCoreLanguages(t *testing.T) {
	...
	coreLanguages := []string{"python", "node", "golang", "rust", "ruby"}
...
func TestContainerLanguages(t *testing.T) {
	...
	containerLanguages := []string{"docker", "docker_image", "conda"}
...
func TestLanguagesByCategory(t *testing.T) {
	...
	categories := map[string][]string{
		"Core":       {"python", "node", "golang", "rust", "ruby"},
		"Mobile":     {"dart", "swift"},
...
func TestListAllLanguages(t *testing.T) {
	...
	for _, test := range allTests {
		...
		case "python", "node", "golang", "rust", "ruby":
		...
		case "docker", "docker_image", "conda":
...
func runSelectedLanguageTests(t *testing.T, languageNames []string, category string) {
	...
	if err := categoryReportGenerator.GenerateReport(); err != nil {
		t.Logf("Warning: Failed to generate category report: %v", err)
	}
...
func isRuntimeAvailable(language string) bool {
	...
	// In practice, you'd check for specific runtime availability
	switch language {
	case "system", "script", "fail", "pygrep":
		return true // These don't require external runtimes
	default:
		// For now, assume most languages are not available in test environment
		return false
	}
}
